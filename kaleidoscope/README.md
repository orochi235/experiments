...
- **Proportional sizes** (Scatter panel) scales parts by their real-world relative size (derived from LDraw dimensions, `rel` in the manifest) instead of rendering every icon at the same box size. Render-time only — preserves tweaks and rides along in shared URLs and saved scenes.
